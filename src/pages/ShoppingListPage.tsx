...
export function ShoppingListPage() {
  ...
  return (
    <PageContainer><div className="mx-auto max-w-3xl"><p className="eyebrow">SHOPPING LIST</p><h1 className="mt-2 text-3xl font-black tracking-tight">買うものリスト</h1><div className="mt-5 grid gap-2 sm:grid-cols-2"><div className="flex items-center gap-3 rounded-xl border bg-white p-4"><Store className="text-teal-700" /><div><p className="text-xs text-slate-500">買い物する店舗</p><p className="font-black">{store.name} {store.branchName}</p></div></div><div className="flex items-center gap-3 rounded-xl border bg-white p-4"><ChefHat className="text-amber-600" /><div><p className="text-xs text-slate-500">つくる料理</p><p className="font-black">{recipe.name}</p></div></div></div>
      <Card className="mt-5 p-4 sm:p-6"><div className="mb-4 flex items-end justify-between"><div><p className="text-xs font-bold text-slate-500">進み具合</p><p className="mt-1 font-black">{checked} / {shoppingList.length}品 完了</p></div><span className="text-2xl font-black text-teal-800">{Math.round((checked / shoppingList.length) * 100)}%</span></div><div className="mb-5 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-teal-600 transition-all" style={{ width: `${(checked / shoppingList.length) * 100}%` }} /></div><ShoppingList items={shoppingList} onToggle={toggleShoppingItem} /><div className="mt-5 flex items-start justify-between gap-4 border-t pt-5"><div><span className="font-bold text-slate-600">{unknownPriceCount > 0 ? "確認できた合計" : "合計"}</span>{unknownPriceCount > 0 && <p className="mt-1 text-xs font-bold text-amber-700">価格不明の商品が{unknownPriceCount}件あります</p>}</div><span className="text-2xl font-black text-teal-900">{formatPrice(knownTotal)}</span></div></Card>
